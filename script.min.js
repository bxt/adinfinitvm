...
(function() {
  var App, BitSet, Button, Component, Grid, GridView, Header, Instructions, Levels, PathySquareDesigns, Solve, Solver, Square, SquareDesigns, SquareView, TriangularSquareDesigns, app,
  ...
  Grid = (function(superClass) {
    ...
    Grid.prototype.newDefault = function() {
      return new Square(0);
    };
  ...
  Solver = (function() {
    ...
    Solver.prototype.solve = function(grid) {
      var counter, i, post, pre, results, square, squares, x, y;
      counter = 0;
      squares = grid.w * grid.h;
      results = [];
      while (!grid.checkAll()) {
        pre = this.pad(counter.toString(4), squares);
        counter = counter + 1;
        post = this.pad(counter.toString(4), squares);
        results.push((function() {
          var j, ref, results1;
          results1 = [];
          for (x = j = 0, ref = grid.w; 0 <= ref ? j < ref : j > ref; x = 0 <= ref ? ++j : --j) {
    ...
    Solver.prototype.pad = function(string, to) {
  ...
  Solve = (function(superClass) {
    extend(Solve, superClass);

    function Solve(main) {
      this.main = main;
      this.clicked = bind(this.clicked, this);
      Solve.__super__.constructor.call(this);
      this.dom.innerHTML = '?';
      this.dom.className = 'button solver';
    }

    Solve.prototype.clicked = function(event) {
      var j, len, ref, results, sV, solver;
      console.log("hm?", this);
      solver = new Solver;
      solver.solve(this.main.gridView.grid);
      console.log("done.");
      ref = this.main.gridView.squareViews;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        sV = ref[j];
