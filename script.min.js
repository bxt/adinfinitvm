...
(function() {
  var DIRECTIONS, Grid, GridView, Levels, Square, SquareDesigns, SquareView, Zoo, fixedLevels, generateLevel, makeGrid, rand, shuffle, zoo,
  ...
  SquareDesigns = (function() {
    function SquareDesigns(size, width, color) {
      this.size = size;
      this.width = width;
      this.color = color;
    ...
    SquareDesigns.prototype.style = function(plus) {
      return "style=\"fill: none; stroke: " + this.color + "; stroke-width: " + this.width + "px; " + plus + "\"";
    };

    SquareDesigns.prototype.styleWhite = function(plus) {
      return "style=\"fill: none; stroke: #fff; stroke-width: " + (this.width * 8 / 6) + "px; " + plus + "\"";
    };
  ...
  Grid = (function() {
    ...
    Grid.prototype.checkAll = function() {
      var j, l, m, quad, ref, ref1, square, x, y;
  ...
  GridView = (function() {
    ...
    GridView.prototype.onDone = function(callback) {
      this.doneHandler = callback;
      return this;
    };

    GridView.prototype.clicked = function() {
      if (this.grid.checkAll()) {
        return typeof this.doneHandler === "function" ? this.doneHandler() : void 0;
      }
    };
  ...
  Levels = (function() {
    ...
    Levels.prototype.get = function(level) {
      return fixedLevels[level] || generateLevel();
    };
  ...
  generateLevel = function(density) {
    var grid, h, j, l, len, m, quad, quads, ref, ref1, w, x, y;
    if (density == null) {
      density = 0.5;
    }
    w = rand(5, 13);
    h = rand(5, 9);
    grid = new Grid(w, h, (function() {
      var j, ref, results;
      results = [];
      for (x = j = 0, ref = w * h; 0 <= ref ? j < ref : j > ref; x = 0 <= ref ? ++j : --j) {
        results.push(new Square(0));
      }
      return results;
    })());
    for (x = j = 0, ref = w; 0 <= ref ? j < ref : j > ref; x = 0 <= ref ? ++j : --j) {
      for (y = l = 0, ref1 = h; 0 <= ref1 ? l < ref1 : l > ref1; y = 0 <= ref1 ? ++l : --l) {
        ...
        for (m = 0, len = quads.length; m < len; m++) {
          quad = quads[m];
  ...
  fixedLevels = {
    ...
    1: makeGrid(6, 6, [0, 0, 6, 12, 0, 0, 0, 4, 5, 7, 8, 0, 6, 11, 15, 15, 10, 12, 3, 10, 15, 15, 14, 9, 0, 2, 13, 5, 1, 0, 0, 0, 3, 9, 0, 0]),
    2: makeGrid(4, 5, [0, 6, 12, 0, 0, 5, 5, 0, 0, 5, 5, 0, 6, 13, 7, 12, 3, 9, 3, 9]),
    3: makeGrid(6, 6, [0, 0, 6, 12, 0, 0, 0, 4, 7, 13, 4, 0, 6, 11, 15, 15, 11, 12, 3, 14, 15, 15, 14, 9, 0, 1, 7, 13, 1, 0, 0, 0, 3, 9, 0, 0]),
    4: makeGrid(5, 9, [4, 6, 10, 10, 12, 1, 5, 6, 14, 9, 6, 13, 5, 1, 0, 7, 11, 13, 2, 1, 5, 4, 5, 6, 12, 7, 14, 15, 6, 8, 7, 6, 14, 11, 13, 5, 5, 2, 3, 13, 1, 3, 8, 3, 9])
  };

  Zoo = (function() {
    function Zoo() {
      this.loadLevel = bind(this.loadLevel, this);
      this.destroyLevel = bind(this.destroyLevel, this);
    ...
    Zoo.prototype.done = function() {
      this.destroyLevel();
      this.level = this.level + 1;
      return this.loadLevel();
    };

    Zoo.prototype.destroyLevel = function() {
    ...
    Zoo.prototype.loadLevel = function() {
      ...
      this.gridView.onDone(this.done);
